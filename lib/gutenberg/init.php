...
add_action( 'wp_enqueue_scripts', 'caetano_enqueue_gutenberg_frontend_styles' );
/**
 * Enqueues Gutenberg front-end styles.
 *
 * @since 2.7.0
 */
function caetano_enqueue_gutenberg_frontend_styles() {
...
add_action( 'enqueue_block_editor_assets', 'caetano_block_editor_styles' );
/**
 * Enqueues Gutenberg admin editor fonts and styles.
 *
 * @since 2.7.0
 */
function caetano_block_editor_styles() {

	$block_editor_settings = genesis_get_config( 'block-editor-settings' );

	wp_enqueue_style(
		'caetano-gutenberg-fonts',
...
add_filter( 'body_class', 'caetano_blocks_body_classes' );
/**
 ...
 */
function caetano_blocks_body_classes( $classes ) {
...
$caetano_block_editor_settings = genesis_get_config( 'block-editor-settings' );

// Adds support for editor font sizes.
add_theme_support(
	'editor-font-sizes',
	$caetano_block_editor_settings['editor-font-sizes']
);

// Adds support for editor color palette.
add_theme_support(
	'editor-color-palette',
	$caetano_block_editor_settings['editor-color-palette']
);

require_once get_stylesheet_directory() . '/lib/gutenberg/inline-styles.php';

add_action( 'after_setup_theme', 'caetano_content_width', 0 );
/**
 * Set content width to match the “wide” Gutenberg block width.
 */
function caetano_content_width() {

	$block_editor_settings = genesis_get_config( 'block-editor-settings' );

	// phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedVariableFound -- See https://github.com/WordPress-Coding-Standards/WordPress-Coding-Standards/issues/924
	$GLOBALS['content_width'] = apply_filters( 'caetano_content_width', $block_editor_settings['content-width'] );
