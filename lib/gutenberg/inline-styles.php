...
add_action( 'wp_enqueue_scripts', 'caetano_custom_gutenberg_css' );
/**
 * Outputs front-end inline styles based on colors declared in config/block-editor-settings.php.
 *
 * @since 2.9.0
 */
function caetano_custom_gutenberg_css() {
...
CSS;
	...
	$css .= caetano_inline_font_sizes();
	$css .= caetano_inline_color_palette();

	wp_add_inline_style( CHILD_THEME_HANDLE . '-gutenberg', $css );

}

add_action( 'enqueue_block_editor_assets', 'caetano_custom_gutenberg_admin_css' );
/**
 ...
 * @since 2.9.0
 */
function caetano_custom_gutenberg_admin_css() {
...
CSS;
	...
	wp_add_inline_style( 'caetano-gutenberg-fonts', $css );
 ...
 */
function caetano_inline_font_sizes() {
 ...
 */
function caetano_inline_color_palette() {
