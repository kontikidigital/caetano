 ...
 * Caetano.
 *
 * This file adds the required WooCommerce setup functions to the Caetano Theme.
 *
 * @package Caetano
 * @author  Max Terbeck
...
add_filter( 'woocommerce_style_smallscreen_breakpoint', 'caetano_woocommerce_breakpoint' );
/**
 ...
 */
function caetano_woocommerce_breakpoint() {
...
add_filter( 'genesiswooc_products_per_page', 'caetano_default_products_per_page' );
/**
 ...
 */
function caetano_default_products_per_page() {

	return 8;

}

add_filter( 'woocommerce_pagination_args', 'caetano_woocommerce_pagination' );
/**
 ...
 */
function caetano_woocommerce_pagination( $args ) {

	$args['prev_text'] = sprintf( '&laquo; %s', __( 'Previous Page', 'caetano' ) );
	$args['next_text'] = sprintf( '%s &raquo;', __( 'Next Page', 'caetano' ) );

	return $args;

}

add_action( 'after_switch_theme', 'caetano_woocommerce_image_dimensions_after_theme_setup', 1 );
/**
 * Defines WooCommerce image sizes on theme activation.
 *
 * @since 2.3.0
 */
function caetano_woocommerce_image_dimensions_after_theme_setup() {
	...
	caetano_update_woocommerce_image_dimensions();

}

add_action( 'activated_plugin', 'caetano_woocommerce_image_dimensions_after_woo_activation', 10, 2 );
/**
 ...
 */
function caetano_woocommerce_image_dimensions_after_woo_activation( $plugin ) {
 ...
 * @since 2.3.0
 */
function caetano_update_woocommerce_image_dimensions() {
...
add_filter( 'woocommerce_get_image_size_gallery_thumbnail', 'caetano_gallery_image_thumbnail' );
/**
 ...
 */
function caetano_gallery_image_thumbnail( $size ) {
