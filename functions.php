 ...
 * Caetano.
 *
 * This file adds functions to the Caetano Theme.
 *
 * @package Caetano
 * @author  Max Terbeck
...
add_action( 'after_setup_theme', 'caetano_localization_setup' );
/**
 * Sets localization (do not remove).
 *
 * @since 1.0.0
 */
function caetano_localization_setup() {

	load_child_theme_textdomain( 'caetano', get_stylesheet_directory() . '/languages' );
...
add_action( 'wp_enqueue_scripts', 'caetano_enqueue_scripts_styles' );
/**
 * Enqueues scripts and styles.
 *
 * @since 1.0.0
 */
function caetano_enqueue_scripts_styles() {
	...
	wp_localize_script(
		...
		caetano_responsive_menu_settings()
	);

	wp_enqueue_script(
		'caetano',
		get_stylesheet_directory_uri() . '/js/caetano.js',
		array( 'jquery' ),
		CHILD_THEME_VERSION,
		true
	);
 ...
 * @since 2.3.0
 */
function caetano_responsive_menu_settings() {

	$settings = array(
		'mainMenu'         => __( 'Menu', 'caetano' ),
		'menuIconClass'    => 'dashicons-before dashicons-menu',
		'subMenu'          => __( 'Submenu', 'caetano' ),
...
add_filter( 'genesis_seo_title', 'caetano_header_title', 10, 3 );
/**
 ...
 */
function caetano_header_title( $title, $inside, $wrap ) {
...
add_action( 'genesis_theme_settings_metaboxes', 'caetano_remove_metaboxes' );
/**
 ...
 */
function caetano_remove_metaboxes( $_genesis_admin_settings ) {
...
add_filter( 'genesis_customizer_theme_settings_config', 'caetano_remove_customizer_settings' );
/**
 ...
 */
function caetano_remove_customizer_settings( $config ) {
...
add_filter( 'wp_nav_menu_args', 'caetano_secondary_menu_args' );
/**
 ...
 */
function caetano_secondary_menu_args( $args ) {
...
add_filter( 'genesis_author_box_gravatar_size', 'caetano_author_box_gravatar' );
/**
 ...
 */
function caetano_author_box_gravatar( $size ) {

	return 90;

}

add_filter( 'genesis_comment_list_args', 'caetano_comments_gravatar' );
/**
 ...
 */
function caetano_comments_gravatar( $args ) {
